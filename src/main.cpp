...
uint256 hashGenesisBlock("0x9e68063dd38154b464114c82d2f2cf57bc5d11b7da3d879b1ebafeebff8292f4");
...
int64 nChainStartTime =  1530728982;    //3/16/2014 00:00:00
...
const string strMessageMagic = "Makhulucoin Signed Message:\n";
...
{
    // Makhulucoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = BLOCKTIME * TORETARGET; // MakhuluCoin: every 1 hours
static const int64 nTargetSpacing = BLOCKTIME; // MakhuluCoin: 1 minutes
...
unsigned int static GetNextWorkRequired_legacy(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Makhulucoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Makhulucoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Makhulucoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        pchMessageStart[2] = 's';
        pchMessageStart[3] = 'c';
        hashGenesisBlock = uint256("0x9e68063dd38154b464114c82d2f2cf57bc5d11b7da3d879b1ebafeebff8292f4");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "makhulucoin";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("0xc0f505250bf41f70752144aaa7355996bd23c8cc8b51b075bce5b587b327cece"));
...
unsigned char pchMessageStart[4] = { 0xfa, 0xbf, 0xb5, 0xda }; // Makhulucoin: increase each by adding 1 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("MakhulucoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("MakhulucoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static MakhulucoinMiner(CWallet *pwallet)
{
    printf("MakhulucoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("makhulucoin-miner");
    ...
    try { while (true) {
        ...
        printf("Running MakhulucoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("MakhulucoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&MakhulucoinMiner, pwallet));
}
