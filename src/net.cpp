...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "Makhulucoin " + FormatFullVersion();
...
static const char *strMainNetDNSSeed[][2] = {
    {"169.159.129.254", "169.159.129.254"},
    {NULL, NULL}
};
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. Makhulucoin is probably already running."), addrBind.ToString().c_str());
        else
